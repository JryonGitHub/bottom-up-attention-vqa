"""
Read in tsv file with pre-trained bottom up attention features and stores it in HDF5 format.
Also store {image_id: feature_idx} as a pickle file.
Hierarchy of HDF5 file:
{'image_features': num_images x num_boxes x 2048 array of features
 'image_bb': num_iamges x num_boxes x 4 array of bounding boxes}
"""
...
FIELDNAMES = ['image_id', 'image_w', 'image_h', 'num_boxes', 'features']
        ...
        for item in reader:
            ...
            if image_id in test_imgids:
                ...
                test_img_features[test_counter, :, :] = np.frombuffer(
                    base64.decodestring(item[None][0]),
                    dtype=np.float32).reshape((item['num_boxes'], -1))
